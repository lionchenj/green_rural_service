...
export default class DueDiligenceItem extends Component {
  handleClick() {
  ...
  setStatus = (status) => {
    switch (status) {
      ...
      case 5:
        return <Text style={[checkList.status,{color: '#ff353d'}]}>未完成</Text>;
      case 6:
        return <Text style={[checkList.status,{color: '#40b19b'}]}>已完成</Text>;
      case 7:
  ...
  render() {
    const { id, term, application_amount, create_time, name,accept_time, status, companyId } = this.props.data;
    return (
        ...
        <View style={checkList.bgi_main}>
          <View style={checkList.align_left}>
            ...
            <View style={checkList.other}>
              <Text style={checkList.date}>{accept_time || ""}</Text>
